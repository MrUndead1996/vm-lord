...
`./rebuild_payload.sh` and `./rebuild_gpu_payload.sh` first, or
...
`ubuntu-24.04` runs `./rebuild_payload.sh` and `./rebuild_gpu_payload.sh`,
uploads the packed pairs -- one display payload and one GPU payload per target
-- and the Windows job downloads them and passes every one to `cargo dist`. The Windows
