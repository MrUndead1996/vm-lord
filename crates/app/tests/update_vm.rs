...
use vmlord_core::{
    AgentStatus, Diagnostic, GpuMode, NetworkMode, RepositoryError, VmRepository, VmState,
...
#[test]
fn update_vm_rejects_running_vm_before_repository_call() {
    let shared = SharedState::default();
    let mut app = WorkspaceApp::new(Box::new(FakeRepository {
    ...
    let error = app.update_vm(update_request()).unwrap_err();

    assert_eq!(
        error.to_string(),
        "VM \"dev\" must be stopped before it can be edited"
    );
