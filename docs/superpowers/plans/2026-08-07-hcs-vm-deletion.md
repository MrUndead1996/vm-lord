...
fn deletes_a_created_vm_completely() {
    ...
    // Both observations are captured before the cleanup below, which would
    // otherwise remove the very directory and mapping file they check and make
    // the assertions pass no matter what deletion did.
